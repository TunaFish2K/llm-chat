...
describe("Store", () => {
  ...
  it("backfills stable branch metadata when migrating a v22 database", () => {
    ...
    expect((migrated.sqlite.prepare("PRAGMA user_version").get() as { user_version: number }).user_version).toBe(32);
    migrated.close();
  });
  ...
  it("migrates v1 data and backfills conversation and generation model fields", () => {
    ...
    expect((store.sqlite.prepare("PRAGMA user_version").get() as { user_version: number }).user_version).toBe(32);
  ...
  it("migrates v12 terminal generations to complete tool result context", () => {
    ...
    const repaired = new Store(path);
    expect((repaired.sqlite.prepare("PRAGMA user_version").get() as { user_version: number }).user_version).toBe(32);
