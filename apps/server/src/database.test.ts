...
describe("Store", () => {
  it.each([39, 40])("opens schema %i without losing messages or retained submission receipts", (version) => {
    ...
    expect(store.sqlite.prepare("PRAGMA user_version").get()).toMatchObject({ user_version: 45 });
    if (version === 40) {
    ...
    try {
      ...
      expect(reopened.sqlite.prepare("PRAGMA user_version").get()).toMatchObject({ user_version: 45 });
  ...
  it("backfills stable branch metadata when migrating a v22 database", () => {
    ...
    expect((migrated.sqlite.prepare("PRAGMA user_version").get() as { user_version: number }).user_version).toBe(45);
    migrated.close();
  });
  ...
  it("migrates v1 data and backfills conversation and generation model fields", () => {
    ...
    expect((store.sqlite.prepare("PRAGMA user_version").get() as { user_version: number }).user_version).toBe(45);
  ...
  it("marks unfinished generations interrupted on restart and rejects future database versions", () => {
    ...
    reopened.sqlite.exec("PRAGMA user_version = 46");
    reopened.close();
  ...
  it("migrates v12 terminal generations to complete tool result context", () => {
    ...
    const repaired = new Store(path);
    expect((repaired.sqlite.prepare("PRAGMA user_version").get() as { user_version: number }).user_version).toBe(45);
