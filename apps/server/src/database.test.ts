...
describe("Store", () => {
  it.each([39, 40])("opens schema %i without losing messages or retained submission receipts", (version) => {
    ...
    expect(store.sqlite.prepare("PRAGMA user_version").get()).toMatchObject({ user_version: 43 });
    if (version === 40) {
      ...
      CREATE INDEX idx_submission_message ON message_submissions(user_message_id);
      CREATE INDEX idx_submission_conversation ON message_submissions(conversation_id);
      CREATE INDEX idx_submission_queue ON message_submissions(queued_message_id);
      PRAGMA user_version = 40;`);
      store.sqlite.prepare("INSERT INTO message_submissions VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
        .run("retained-receipt", "send", "original-hash", conversation.id, generation.userMessageId!, generation.assistantMessageId, generation.generationId, null, 1);
    }
    ...
    try {
      ...
      expect(reopened.sqlite.prepare("PRAGMA user_version").get()).toMatchObject({ user_version: 43 });
  ...
  it("backfills stable branch metadata when migrating a v22 database", () => {
    ...
    expect((migrated.sqlite.prepare("PRAGMA user_version").get() as { user_version: number }).user_version).toBe(43);
    migrated.close();
  });
  ...
  it("migrates v1 data and backfills conversation and generation model fields", () => {
    ...
    expect((store.sqlite.prepare("PRAGMA user_version").get() as { user_version: number }).user_version).toBe(43);
  ...
  it("keeps a conversation reasoning override stable across sends and retries", () => {
    ...
    store.updateModel(model.id, {
      reasoningEffortsOverride: ["high", "xhigh", "max"],
  ...
  it("marks unfinished generations interrupted on restart and rejects future database versions", () => {
    ...
    reopened.sqlite.exec("PRAGMA user_version = 44");
    reopened.close();
  ...
  it("migrates v12 terminal generations to complete tool result context", () => {
    ...
    const repaired = new Store(path);
    expect((repaired.sqlite.prepare("PRAGMA user_version").get() as { user_version: number }).user_version).toBe(43);
...
it("persists translated error metadata without changing raw history", () => {
  const store = createStore();
  seedModel(store);
  const conversation = store.createConversation({ systemPrompt: "" });
  const generation = store.createMessageGeneration(conversation.id, "用户原文");
  const descriptor = { key: "error.model_not_found" as const };
  store.upsertToolCall(generation.generationId, { id: "localized-call", name: "workspace_read_file", arguments: '{"path":"原文"}' }, 0, 0, false);
  store.updateToolCall("localized-call", { error: "模型不存在", errorI18n: descriptor, approvalState: "failed" });
  store.finishGeneration(generation.generationId, "failed", { code: "model_not_found", message: "模型不存在", i18n: descriptor });
  expect(store.getGeneration(generation.generationId)?.error).toEqual({ code: "model_not_found", message: "模型不存在", i18n: descriptor });
  expect(store.getToolCall("localized-call")).toMatchObject({ error: "模型不存在", errorI18n: descriptor });
  store.sqlite.prepare("UPDATE generations SET error_i18n_json = NULL WHERE id = ?").run(generation.generationId);
  expect(store.getGeneration(generation.generationId)?.error).toEqual({ code: "model_not_found", message: "模型不存在" });
});
