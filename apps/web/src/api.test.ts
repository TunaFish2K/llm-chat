...
describe("api client", () => {
  ...
  it("constructs every endpoint with its expected method and JSON body", async () => {
    const cases: Array<[() => Promise<unknown>, string, string, unknown?]> = [
      ...
      [() => api.createConversation({ systemPrompt: "sys" }), "/api/conversations", "POST", { systemPrompt: "sys" }],
      [() => api.startConversation({ text: "hi", modelId: "m", contextPolicy: "trim" }), "/api/conversations/start", "POST", { text: "hi", modelId: "m", contextPolicy: "trim" }],
