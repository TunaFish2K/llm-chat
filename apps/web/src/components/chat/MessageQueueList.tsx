...
export function useMessageQueue(conversationId?: string) {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener("llm-chat:message-queue", update); window.removeEventListener("llm-chat:queue-reconnect", update); };
  }, [reload, conversationId]);
  return { items, reload };
}

export function MessageQueueList({ conversationId, items, reload }: {
  conversationId: string | undefined; items: QueuedMessageDto[]; reload: () => Promise<void>;
}) {
