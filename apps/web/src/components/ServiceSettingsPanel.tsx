...
export function ServiceSettingsPanel({ kind }: { kind: "search" | "image" }) {
  ...
  const save = async (input: ServiceSettingsInput) => {
    setBusy(true);
    try { setData(await endpoints.updateServiceSettings(input)); setKeys({}); toast("success", "已保存推荐顺序与配置"); }
    ...
    <fieldset disabled={busy} className="service-fields"><ol className="service-list">{items.map((item, index) => <li key={item.id}>
      <div className="service-heading">
        ...
        <div className="service-order">
          <button className="icon-button" title="上移" aria-label={`上移 ${item.id}`} disabled={busy || index === 0} onClick={() => move(index, -1)}><ArrowUp size={17} /></button>
          <button className="icon-button" title="下移" aria-label={`下移 ${item.id}`} disabled={busy || index === items.length - 1} onClick={() => move(index, 1)}><ArrowDown size={17} /></button>
        </div>
      </div>
