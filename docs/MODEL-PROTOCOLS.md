# 模型协议

在“设置 → 连接”中编辑模型，可选择“自动识别”或手动指定 OpenAI Chat Completions、OpenAI Responses、
Anthropic Messages。同一连接下的模型可以使用不同协议，共用连接地址、API Key 和秘密请求头。
模型列表、选择器和编辑页显示实际使用的协议。

自动选择顺序是：手动指定 → 目录识别 → OpenCode Go 官方模型映射 → 连接默认协议。
例如，OpenCode Go 的 `grok-4.6` 使用 `/responses`，`minimax-m3` 使用 `/messages`，
`glm-5.3` 使用 `/chat/completions`。映射仅适用于确切的提供商和模型 ID，未知模型沿用连接默认值。
官方端点来源：[OpenCode Go endpoints](https://opencode.ai/docs/go/#endpoints)。

“发现模型”从 models.dev 的确切提供商和模型条目读取模型级 `provider.npm`：
`@ai-sdk/openai` 对应 Responses，`@ai-sdk/openai-compatible` 对应 Chat Completions，
`@ai-sdk/anthropic` 对应 Messages。跨提供商的相似名称只用于补充能力信息，不用于推断协议。
目录暂时不可用时保留已有识别结果。更换模型 ID、所属连接，或修改连接的提供商、地址后清除旧识别结果。

只修改协议不会关闭模型参数的目录管理。重新发现或恢复目录管理不会覆盖手动协议；手动管理参数的模型
也会更新自动协议。需要恢复自动选择时，将“模型协议”改为“自动识别”。

普通生成、工具续轮、排队消息、摘要和备用识图使用同一协议解析规则。生成记录保存实际使用的协议，
审批恢复沿用该记录，避免生成途中修改设置导致切换协议。请求失败不会自动改用其他协议重试。

API 的模型 `protocol` 字段可省略或设为 `null`。新模型省略时使用自动模式；更新时省略保留原值，
传 `null` 清除手动覆盖。`detectedProtocol` 只读，由服务维护。旧客户端和离线记录缺少这些字段仍可读取。
SQLite v42 增加两个可空列，部署和回滚步骤见[运维手册](DEPLOYMENT.md#模型协议升级v42)。

## 原生推理档位

快捷推理菜单、Agent 设置和高级会话设置使用同一套档位规则。优先使用模型的手动列表，其次使用
确切提供商与模型 ID 的目录声明。跨提供商的相似名称不用于确定原生档位。目录故障保留已有识别结果；
更换模型 ID、所属连接，或修改连接提供商、地址后清除旧识别结果。

档位按声明顺序显示，保留 `minimal`、`none`、`default` 等原生字符串。模型没有声明档位时，只提供
“默认”，不显示未知或不支持提示；可以在模型编辑页将“推理档位来源”改为“手动”，
每行填写一个原生值。空列表只提供默认选项。重新发现不会覆盖手动列表，修改列表也不会关闭其他参数的目录管理。

“跟随 Agent”表示会话不设置覆盖。“默认”表示不发送推理参数；原生 `none` 或 `default`
则按原值发送。用户选择作为原始偏好保存；前后端按当前模型解析有效档位，自动适配不回写偏好。
原值受支持时保留，否则按 `none < minimal < low < medium < high < xhigh < max` 向上取最近支持档，
超过最高档时取最高可比较档。自定义名称无法比较且不受支持时，取模型声明列表的最后一项。
例如偏好 `medium`，模型只支持 `low` 和 `xhigh` 时使用 `xhigh`；偏好 `max`，模型最高支持 `xhigh`
时使用 `xhigh`，切回支持 `max` 的模型后恢复。原生选项的显示顺序不因比较规则改变。

快捷滑条只包含“默认”和模型支持的原生档位，直接选中有效值，不展示不支持提示或无效选项。
发送栏不显示“跟随 Agent”开关或提示，按钮提示只显示有效档位。未手动选档时沿用 Agent 配置，
手动选择后保存会话覆盖；可在高级会话设置中选择“跟随 Agent”以移除覆盖。Agent 和高级会话编辑器也显示
适配结果，只有用户明确选择才更新偏好。没有原生档位、只有推理开关或 token 预算的模型，新生成使用默认，
不发送推理参数，也不生成虚构档位。旧配置的 `reasoningEffort` 在新生成时同样参与适配。

新生成、队列发送和重试在保存快照前解析有效档位，快照的执行配置与请求设置使用同一个结果。
已保存的生成快照、工具续轮和审批恢复沿用原值，包括旧手动 Thinking 预算，不按当前模型重新适配。

### API 和历史兼容

Agent 执行配置、会话覆盖和生成设置增加可选 `reasoningSelection`：

```json
{ "mode": "default" }
```

或原生值：

```json
{ "mode": "effort", "value": "minimal" }
```

会话省略该字段和旧 `reasoningEffort` 字段时继承 Agent。新的选择优先于同一层的旧字段；会话显式设置
优先于 Agent。新界面修改会话选择时移除旧字段，修改 Agent 时将旧字段保留为兼容占位值 `none`。
已有新选择的 Agent 若收到缺少新字段的完整执行配置，或已有新会话选择被旧字段替换，返回
`client_update_required`，要求刷新页面。

模型增加 `reasoningEffortsOverride`（`null` 为自动，数组为手动列表）和只读的
`detectedReasoningEfforts`。更新模型时省略手动列表保留原值；传 `null` 恢复自动。识别结果独立于
模型其他参数的目录管理状态。模型声明不支持推理时，菜单只提供默认值。

旧 `reasoningEffort: "none"` 仍表示省略参数；旧预算快照沿用原有换算。新选择保存在生成快照中，
工具续轮和审批恢复不重新解释当前模型的档位。旧历史和离线记录仍可读取，不改写历史请求。
SQLite v43 增加模型手动列表和自动识别列表两列，并从旧数据中确切匹配的目录记录回填识别列表。
新建 Agent 使用默认，已有 Agent 和会话设置保留。
