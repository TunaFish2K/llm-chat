...
新内置调用额外保存 `presentation.builtin: { name, version: 1 }`。服务端和浏览器共用 `@llm-chat/i18n/tool-presentation` 中的纯格式器，仅根据已保存的参数、结果和错误生成说明。浏览器按当前语言翻译表头、状态和数量，不改变原始数据或模型上下文。

只有版本和名称匹配的内置描述才参与本地化。未知版本、缺少描述的旧记录、插件和 MCP 调用继续使用已保存的 Markdown 或原始数据。查看历史不加载插件，也不再次运行插件格式器。错误可另带 `errorI18n`；`error` 原文仍保留。
