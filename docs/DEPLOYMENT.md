...
普通聊天附件支持单文件 2 GiB、单条消息合计 4 GiB，最多 8 个附件。浏览器用 4 MiB 分块上传，
临时文件写入 `dataDir/file-uploads`，SQLite v46 保存元数据和已确认偏移。上传完成后服务端流式校验
SHA-256，再发布到现有文件资产目录；下载也按流或 Range 读取，不将整个文件载入服务端内存。

同一标签页最多同时上传两个文件，切换会话或设置后继续。页面刷新后，在上传列表重新选择原文件，
通过内容校验后从服务端已确认位置继续。服务重启同样保留进度。连续 24 小时无活动的上传任务过期；
应用启动时及每小时清理过期任务和遗留临时文件。取消上传会删除其临时文件。

反向代理必须允许 `PATCH`、`DELETE` 及至少 4 MiB 的请求体，并允许上传所需的请求时长。
不必把整个站点的单请求限制提高到 2 GiB。代理返回错误时，上传列表保留进度供重试。

为临时文件、已完成的文件资产和会话附件工作区预留磁盘空间。会话附件副本优先使用文件系统的写时复制；
不支持时会实际复制，因此发送和分叉可能额外占用一份文件大小。磁盘不足会报错；释放空间后重试。
图片、角色卡等需要完整解析的入口仍执行各自较小的限额。

`pnpm test:upload:large` 在临时目录运行独立服务，实际上传并下载 2 GiB，校验内容和 Range/HEAD，
记录服务端内存峰值。临时目录需至少 2 GiB 可用空间；测试结束后清理数据。

升级到 v46 前备份完整 `dataDir`。上传临时文件与数据库必须一起备份和恢复；只恢复数据库无法恢复进度。
回滚到仅支持 v45 的版本时，恢复升级前的完整备份。
...
语义保留。旧客户端修改新格式的推理设置时需要刷新页面。未知档位的模型只提供“默认”，需要时
...
- `image-assets/` 下的不可变文件资产、`attachment-workspaces/` 下的会话副本，以及 `file-uploads/` 下的续传临时文件；
- 工作目录和其他服务端状态。
...
覆盖率门槛为全局语句和行 95%、分支 90%、函数 90%；关键文件另有独立门槛。覆盖范围保持在
`vitest.config.ts` 中，页面交互由 Playwright 验证。定位不稳定场景时使用
`pnpm exec playwright test <测试文件> --repeat-each=3 --retries=0`，确认连续通过。

E2E 在每个用例后清理新建会话、Agent 和连接。需要拦截 API 的用例应禁用 Service Worker，避免
请求绕过拦截；离线缓存用例保留真实 Service Worker。WebKit 在 Linux 下通过临时本机代理切断
浏览器及 Worker 的上游连接，避免原生断网模拟直接拒绝缓存导航；测试 API 客户端仍可访问临时服务。
...
SQLite v45 增加容器资源记录和环境预载选择；v44 的已有环境自动保留。升级前停止服务并备份数据。回滚到旧服务需要恢复对应版本的数据库备份；资源缓存和引擎内镜像应随应用数据一起保存。
