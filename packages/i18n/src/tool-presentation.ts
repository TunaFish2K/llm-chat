...
interface ToolMarkdown { summary?: string; detail?: string }
interface ToolResultFormatInput { input: Record<string, unknown>; output: string | null; error: string | null }

/** Version 1 only reads stored arguments and results. It never executes a tool. */
export function localizedToolFormatters(name: string, locale: Locale) {
  ...
  function clip(value: string, bytes: number): string {
    if (new TextEncoder().encode(value).length <= bytes) return value;
    const suffix = t("tool.truncated");
    return new TextDecoder().decode(new TextEncoder().encode(value).subarray(0, bytes - new TextEncoder().encode(suffix).length)).replace(/\uFFFD$/, "") + suffix;
  }

  function normalizeToolMarkdown(value: ToolMarkdown): ToolMarkdown {
    const summary = value.summary?.trim() ?? "";
    const detail = value.detail?.trim() ?? "";
    const suffix = t("tool.truncated");
    return {
      ...(summary ? { summary: Array.from(summary).length > 512 ? Array.from(summary).slice(0, 512 - Array.from(suffix).length).join("") + suffix : summary } : {}),
  ...
  function fields(value: unknown, depth = 0): string {
    ...
    if (Object.keys(value).length && Object.values(value).every((item) => !object(item) && !Array.isArray(item) && !(typeof item === "string" && item.includes("\n")))) {
      return t("tool.fields") + Object.entries(value).map(([key, item]) => `| ${inline(key)} | ${inline(item)} |`).join("\n");
    }
  ...
  function builtinToolFormatters(name: string) {
    if (!builtinNames.has(name)) return undefined;
    return {
      formatArguments(input: Record<string, unknown>) {
        const summary = inline(input.command ?? input.code ?? input.query ?? input.path ?? input.url ?? input.prompt ?? [input.action, input.id ?? input.task_id ?? input.name].filter(Boolean).join(" · ")) || t("tool.no_arguments_summary");
      ...
      formatResult({ input, output, error }: ToolResultFormatInput) {
        const value = parse(output ?? "");
        let detail = fields(value);
        let summary = error ? t("tool.failed", { error: inline(error) }) : Array.isArray(value) ? t("tool.items", { count: value.length }) : t("tool.completed");
        if (object(value)) {
          const count = Object.values(value).find(Array.isArray);
          summary = error ? summary : inline(value.status ?? (value.exitCode !== undefined ? t("tool.exit_code", { code: value.exitCode }) : count ? t("tool.items", { count: count.length }) : t("tool.completed")));
  ...
  return builtinToolFormatters(name);
}
