...
test.describe("应用外壳", () => {
  ...
  test("主题切换只在当前浏览器持久化", async ({ page, request }) => {
    ...
    await expect(page.locator('meta[name="theme-color"]')).toHaveAttribute(
      "content", next === "light" ? "#f5f7f5" : "#0d100e"
    );
    await page.reload();
    await expect(page.getByLabel("主题")).toHaveValue(next);
    await expect(page.locator("html")).toHaveAttribute("data-theme", next);
  ...
  test("桌面收起栏在所有主页面保持可导航和可展开", async ({ page }) => {
    ...
    for (const theme of [
      ...
      { theme: "dark", accentColor: "#018EEE" }
    ]) {
      await page.getByLabel("主题", { exact: true }).selectOption(theme.theme);
      await page.getByRole("button", { name: theme.accentColor ? "蓝色" : "默认橙色", exact: true }).click();
