...
import { agentInput, api, APP_URL, reasoningModel } from "./helpers.mjs";
...
test("通用设置链接到所选 Agent，生成配置只修改该 Agent", async ({ page, request }) => {
  ...
  const second = await api(request, APP_URL, "POST", "/api/agents", agentInput(`settings-second-${Date.now()}`, model.id));
  try {
    ...
    await page.getByLabel("推理档位", { exact: true }).selectOption("effort:high");
    await page.getByRole("button", { name: "保存修改", exact: true }).click();
    await expect(page.getByRole("button", { name: "已保存", exact: true })).toBeVisible();
    const saved = await api(request, APP_URL, "GET", `/api/agents/${second.id}`);
    expect(saved.execution).toMatchObject({ baseSystemPrompt: "Only this Agent's instructions", contextPolicy: "full", reasoningSelection: { mode: "effort", value: "high" }, modelId: model.id });
